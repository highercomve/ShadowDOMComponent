...
},{"./source/index.js":6}],2:[function(require,module,exports){
/**
 * @license
...
function SetListeners() {
  ...
  return {
    attachedCallback: function attachedCallback() {
      console.log('Element was attached', this);
      console.log('Element state on attach', JSON.stringify(this.state));
      this.onMount();
...
function viewCompile(view) {
  if (typeof view === "string") {
    ...
    return function compile() {
      ...
      var viewResult = view.bind(this)();
      if (Array.isArray(viewResult)) {
        viewResult.forEach(function (appendableView) {
          temporalContainer.appendChild(appendableView);
        });
      } else {
        temporalContainer.appendChild(viewResult);
      }
      return temporalContainer.innerHTML;
    };
  }
}

function renderFactory(view) {
  var viewCompiler = viewCompile(view);
  return {
    render: function render() {
      this.innerHTML = viewCompiler.bind(this)();
    }
  };
}
...
},{"./builder":3,"./exceptions":5}],5:[function(require,module,exports){
'use strict';
  ...
  if (typeof child !== 'undefined') {
    if (Array.isArray(child)) {
      child.forEach(function (appendableView) {
        element.appendChild(appendableView);
      });
    } else {
...
},{"./create":4,"webcomponents-lite":2}],7:[function(require,module,exports){
'use strict';
...
var state = {
  items: [{ text: "Primera tarea", ready: false }]
};
...
var TodoItem = _shadowComponent2.default.ComponentFactory({
  elementName: "todo-item",
  state: defaultItem,
  template: "<content></content>",
  view: ['<input class="text" type="text" value="${this.state.text}">', '<input class="checkbox" type="checkbox" ${(this.state.ready) ? "checked":""}>'].join(''),
...
var TodoList = _shadowComponent2.default.ComponentFactory({
  ...
  view: function view() {
    return this.state.items.map(function (item, index) {
      ...
      return _shadowComponent2.default.createElement('todo-item', itemState);
    });
  }
});
...
var TodoApp = _shadowComponent2.default.ComponentFactory({
  ...
  view: function view() {
    return [_shadowComponent2.default.createElement('todo-list', { items: this.state.items })];
  }
});

document.getElementById('TodoApp').appendChild(TodoApp);

},{"shadow-component":1}]},{},[7]);
